...
eig_try <- function(cent = FALSE, df, ngame, nsamp){
...
calc_pval <- function(eig, quan, n_MC){
  set.seed(520)
  n_eig <- length(eig)
  ...
  return(list(p_val = 1 - stats::ecdf(MC)(Z), 
              quantile = quantile(MC, quan)))
}

ZZ <- calc_Z(df = df_equ, pA = "phat_A", pB = "phat_B", Y = "Y", nsamp = nsamp, ngame = ngame)
eigg <- eig_try(df = temp, ngame = ngame, nsamp = nsamp)
oh <- calc_pval(eig = eigg, quan = c(0.90, 0.95, 0.99), n_MC = N_MC)
